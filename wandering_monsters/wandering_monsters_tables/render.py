...
def render():
    ...
    for level in enumerate(levels):
        ...
        for difficulty in difficulties:
            output.append( f"#### {difficulty}\n".title() )
            output.append( f"| How Many | Challeng Rating |" )
            output.append( f"|:---------------|:----- |" )
            rows = rows + 1
            rows = rows + 1
            rows = rows + 1
            for cr in crs:
                try:
                    output.append( f"| &emsp;  {level[1][difficulty][cr]} | CR {cr} |" )
                    rows = rows + 1
                except:
                    pass
            output.append( f"\n" )
    ...
    for file in files:
        ...
        output.append( f"{{{{wide" )
        output.append( f"### {title}".title() )
        output.append( f"}}}}" )
        output.append( "| d100 | Monster |" )
        output.append( "|:------------------| :--- |" )
        row_num = 0
        col_num = 1
        for group in data:
            ...
            if num_per_group > 0:
                ...
                for row in data[group]:
                    ...
                    output.append( f"| {i + last} - {this} | {row['name']} |" )
                    last = this 
